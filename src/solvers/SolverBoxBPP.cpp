...
namespace
{
    ...
    static inline void initializeBounds(std::vector<Contact*>& contacts, Eigen::VectorXf& lower, Eigen::VectorXf& upper)
    {
        for (auto c : contacts)
        {
    ...
    static inline void buildRHS(const std::vector<Joint*>& joints, const std::vector<Contact*>& contacts, float h, Eigen::VectorXf& b)
    {
        ...
        for (auto j : joints)
        {
            ...
            b.segment(j->idx, dim) = -hinv * gamma * j->phi;
        ...
        for (auto c : contacts)
        {
            ...
            b.segment(c->idx, dim) = -hinv * gamma * c->phi;
    ...
    static inline void buildMatrix(const std::vector<Joint*>& joints, const std::vector<Contact*>& contacts, float h, Eigen::MatrixXf& A)
    {
        ...
        for (auto c : contacts)
        {
            ...
            A.block(c->idx, c->idx, dim, dim) = 1e-7f * Eigen::MatrixXf::Identity(dim, dim);
            A(c->idx, c->idx) += eps;
    ...
    // 
    static inline unsigned int pivot(Eigen::VectorXi& idx, Eigen::VectorXf& x, const Eigen::VectorXf& l, const Eigen::VectorXf& u, const Eigen::VectorXf& v, bool singlePivot)
    {
...
void SolverBoxBPP::solve(float h)
{
    ...
    if (N > 0)
    {
        ...
        initializeBounds(contacts, lower, upper);
        ...
        for (int i = 0; i < dim; ++i)
        {
            if (idx(i) == kFree && x(i) < lower(i))
            {
                x(i) = lower(i);
                idx(i) = kLower;
            }
            else if (idx(i) == kFree && x(i) > upper(i))
            {
                x(i) = upper(i);
                idx(i) = kUpper;
            }
        ...
        const unsigned int singlePivotIter = m_maxIter - 5;
        ...
        for (int k = 0; k < 6; ++k)
        {
            ...
            for (unsigned int iter = 0; iter < m_maxIter; ++iter)
            {
                ...
                // Pivot.
                // 
                const int numPivots = pivot(idx, x, lower, upper, v, (iter > singlePivotIter));
                if (numPivots == 0)
                    break;      // Done
            }
